<?php 
	...
	{		
		...
		case RECORD_NAV_COMMANDS::SAVE:
			...
			{						
				// Call update stored procedure.
				$query->set_sql('{call account_sub_role_update(@fk_id = ?,														 
														@xml = ?,
														@log_update = ?,
														@log_update_by = ?,
														@log_update_ip = ?)}');
	...
	<script>
		...
		{						
			$(".ec").append(
				'<tr>'
					+'<td>'
						+'<select name="sub_role_role[]" id="sub_role_role_'+ $temp_int +'" class="form-control">'
						+'<?php echo $role_list_options; ?>'
						+'</select>'																		
					+'</td>'					
					+'<td colspan="2">'
						+'<input type="hidden" name="sub_role_id[]" id="sub_role_id_js_'+$temp_int+'" value="<?php echo DB_DEFAULTS::NEW_ID; ?>" />'
