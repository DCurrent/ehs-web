<?php 
	...
	{		
		...
		case RECORD_NAV_COMMANDS::LISTING:
			...
			// Direct to listing.				
			header('Location: audit_question_category_list.php');
			break;
		...
		case RECORD_NAV_COMMANDS::DELETE:						
			...
			$query->set_sql('{call audit_question_category_delete(@id = ?,													 
									@update_by	= ?, 
									@update_ip 	= ?)}');
		...
		case RECORD_NAV_COMMANDS::SAVE:
			...
			$query->set_sql('{call audit_question_category_update(@id 				= ?,														 
													@label 			= ?,
													@details 		= ?,													 
													@log_update_by	= ?, 
													@log_update_ip 	= ?)}');
	...
	//////	
	$query->set_sql('{call audit_question_category(@id = ?)}');
	$params = array(array($_main_data->get_id(), SQLSRV_PARAM_IN));
...
<html lang="en">
    <head>
        ...
        <title><?php echo APPLICATION_SETTINGS::NAME; ?>, Category Detail</title>        
    ...
    <body>    
        <div id="container" class="container">            
            ...
            <div class="page-header">           
                <h1>Audit Question Category Entry<?php if($_main_data->get_label()) echo ' - '.$_main_data->get_label(); ?></h1>
                <p>This screen allows for adding and editing categories for audit questions.</p>
            </div>
            ...
            <div class="col-sm-10">
                ...
                <div class="form-group">
                	...
                	<div class="col-sm-10">
                    	<textarea class="form-control" rows="5" name="notes" id="notes"><?php echo $_main_data->get_details(); ?></textarea>
                	</div>
                </div>
