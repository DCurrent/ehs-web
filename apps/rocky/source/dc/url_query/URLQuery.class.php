<?php
	...
	final class URLQuery
	{
		...
		public function set_data($key, $value) 
		{
			...
			//echo 'url_query->set_data('.$key.', '.$this->data[$key].')';
		}
		...
		private function getStringInternal($HTML = FALSE) 
		{
			...
			foreach ($this->data as $key => $value) 
			{
				if (!is_array($value)) 
				{
					...
					{
						$result .= urlencode($key)."=".urlencode($value).'&'; // &amp; mark.
					}
				}
				else
				{
					...
					{
						$result .= urlencode($key)."[]=".urlencode($value_element).'&'; // &amp mark.
					}
				}
			}
