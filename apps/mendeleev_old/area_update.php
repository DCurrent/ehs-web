	...
	{		
		...
		case RECORD_NAV_COMMANDS::SAVE:
			...
			$params = array(array($access_obj->get_account(), 			SQLSRV_PARAM_IN),
						array($_main_data->get_facility(), 			SQLSRV_PARAM_IN),
						...
						array($room_List,					SQLSRV_PARAM_IN));
    ...
    <body>    
        <div id="container" class="container">            
            ...
            <div class="page-header">           
                ...
                <h2>Instructions</h2>
                <p>To enter areas (labs/rooms/benches) you have updated, first fill in the fields below and select a facility. One you have selected a facility, press <span class="glyphicon glyphicon-plus btn btn-success btn-xs"></span> below to add areas. You may add as many as areas as you like. When you are ready, press <span class="glyphicon glyphicon-floppy-disk btn btn-warning btn-xs"></span> to submit your entry.</p>
            </div>
            
            <?php 	if($dialog_no_areas === TRUE)
					{
			?>
            	<p class="bg-danger lead">You need to add at least one area before submitting.</p>
            <?php
					}
			?> 
            ...
            <form class="form-horizontal" role="form" method="post" enctype="multipart/form-data">           
                ...
                <div class="form-group">                    	
                    <div class="col-sm-offset-2 col-sm-10">
                        <fieldset>
                            <legend>Areas, Labs, &amp; Rooms</legend>                                
                            <table class="table table-striped table-hover" id="tbl_area"> 
                                <thead>
                                </thead>
                                <tfoot>
                                </tfoot>
		...
		<script>
			...
			// Enable adding of areas after facility is selected.
			var $facility 		= $('#facility'); 
			var $row_add_area	= $('#row_add_area');
			
			$facility.change(function(event)
			{
				if($facility.val())
				{
					$row_add_area.attr('disabled', false);
				}
				else
				{
					$row_add_area.attr('disabled', true);
				}
			});

			function deleteRow_area(row)
			{
				var i=row.parentNode.parentNode.rowIndex;
				document.getElementById('tbl_area').deleteRow(i);
			}
			
			// Inserts a new table row on user request.
			function insRow_area(event)
			{
				var $guid = null;
					...
					'<tr>'
						...
						+'<td>'         
							+'<select '
								+'name 		= "area_list[]" '
								+'id		= "area_list_' + $guid + '" '
								+'data-source-url = "../../libraries/inserts/room_by_rm_number.php" '
								+'class		= "form-control">'
							+'</select>'
						+'</td>'
						...
						+'<td>'								
							...
							+'name	="area_row_del" '
							+'id	="area_row_del_' + $guid + '" '
							+'title	="Remove this item." '
							+'onclick="deleteRow_area(this)"><span class="glyphicon glyphicon-minus"></span></button>'                                  
						+'</td>'
						                                            
					+'</tr>');
					
				options_update(event, null, '#area_list_' + $guid);
			}
