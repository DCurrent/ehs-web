<!--Include: <?php echo __FILE__ . ", Last update: " . date(DATE_ATOM,filemtime(__FILE__)); ?>-->
<nav id="nav_sub" class="nav_sub">
    <ul id="toplevel">
      <li><a href="#" onclick="return false" class="fly">Fire Policy</a>
        <ul>
          <li><a href="ada_evac.html">ADA Evacuation Policy</a></li>
          <li><a href="bicycle.php">Bicycle In Buildings</a></li>
          <li><a href="electrical.html">Electrical Policy</a></li>
          <li><a href="evacuation.html">Evacuation Policy - Fire Alarms</a></li>
          <li><a href="micro.html">Microwave Policy<br /></a></li>
          <li><a href="natgas.html">Natural Gas Emergency Response Procedures</a>
          <li><a href="../docs/pdf/fs_life_safety_policy.pdf">On-Campus Student Housing</a></li>
          <li><a href="openfire.html">Open Flame Policy</a></li>
          <li><a href="floatpolicy.html">Parade Float Policy</a></li>
          <li><a href="recyclepolicy.html">Recycling Container Policy</a></li>
		  <li><a href="shell_space.php">Shell Space Usage</a></li>
          <li><a href="heater.html">Space Heater Policy</a></li>
          <li><a href="tentpolicy.html">Tent Policy</a></li>
          <li><a href="vending.html">Vending Machine Policy</a></li>
          <li><a href="/docs/pdf/fire_property_acceptance_form.pdf" target="_blank">Property Acceptance Form</a></li>
        </ul>
      </li>
      <li><a href="#" onclick="return false" class="fly">Procedures &amp; Processes</a>
        <ul>          
          <li><a href="plan01.html">Construction Plan Review/Approval Process</a></li>
          <li><a href="ukfire1.html">Emergency Evacuation Plans</a></li>
          <li><a href="/apps/flashpoint/alarm_entry.php">Fire Alarm Report</a></li>
          <li><a href="drill.php"> Fire Drill Report</a></li>          
          <li><a href="flstpol1.html">Standard for Solvents in Labs</a></li>
        </ul>
      </li>
      <li><a href="#" onclick="return false" class="fly">Reports &amp; Logs</a>
        <ul>
          <li><a href="/apps/flashpoint/incident_log.php">Campus Fire Log</a></li>
          <li><a href="drill.php">Drill Report Entry</a></li>
          <li><a href="alarm.php">Fire/False Alarm Report Entry</a></li>
        </ul>
      </li>
      <li><a href="#" onclick="return false" class="fly">Safety Guides</a>
        <ul>
          <li><a href="evac.html">Assisting People with Physical Impairments</a></li>
          <li><a href="earthquake.html">Earthquake Safety</a></li>
          <li><a href="//www.ilpi.com/msds/ref/nfpa.html">Hazardous Material Identification Guide</a></li>
          <li><a href="tornado.html">Severe Weather</a></li>
        </ul>
      </li>
       
      <li><a href="#" onclick="return false" class="fly">Useful Links</a>
        <ul>
          <li><a href="//www.ilpi.com/safety/extinguishers.html">Fire Safety in a Chemistry Lab</a></li>
          <li><a href="../docs/xls/bio_pi_req_matrix.xls">PI Requirements Matrix</a></li>
        </ul>
      </li>
    </ul>
</nav><!--/nav_sub-->
<!--/Include: <?php echo __FILE__; ?>-->