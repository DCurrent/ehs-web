<?php
		...
		foreach($line_all as $line)
		{
			...
			{				
				foreach ($preset as $preset_ele)
				{
					...
					if($line->id == $preset_ele) $result .= ' selected';
				}
			}
			else
			{				
			...
			$result .= '>'.$line->desc_title.'</option>'.PHP_EOL;
		}
	...
	*/		
	...
	// Query for results.
	$query->set_sql('SELECT DISTINCT id, desc_title FROM tbl_class_train_parameters ORDER BY desc_title');
