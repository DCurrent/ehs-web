	...
	*/		
	...
	switch ($post->sql_from)
	{
		case DEPARTMENT_OPTIONS::FROM_EHS_STAFF:
			$from = 'FROM dbo.tbl_staff INNER JOIN dbo.vEbarsDepartments ON dbo.tbl_staff.department = dbo.vEbarsDepartments.DeptCode';
			break;
		default:
			$from = 'FROM vEbarsDepartments';
			break;			
	}
	...
	{	
		...
		// Query for results.
		$query->set_sql('SELECT DISTINCT DeptCode, DeptName '.$from.'  WHERE DeptCode LIKE ? ORDER BY DeptName');
		...
		// Query for results.
		$query->set_sql('SELECT DISTINCT DeptCode, DeptName '.$from.' WHERE DeptCode NOT LIKE ? ORDER BY DeptName');
		$query->set_params(array(GROUP));
		$query->query();
		$line_all = $query->get_line_object_all();		 
	...
	{
		// Query for results.
		$query->set_sql('SELECT DISTINCT DeptCode, DeptName '.$from.' ORDER BY DeptName');
		$query->query();
		$line_all = $query->get_line_object_all();		 
