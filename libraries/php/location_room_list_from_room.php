<?php	
	...
	class class_location
	{
		...
		private function get_building_from_room()
		{
			...
			$parms = array($this->room_barcode);
			
			$query->set_params($parms);
			$query->query();
		...
		private function create_floor_list_from_building()
		{
			...
			$parms = array($this->building_code);
		...
		private function create_room_list_from_room	()
		{
			...
			$this->building_code = get_building_from_room();
			
			$_obj_floor_list = create_floor_list_from_building();
				...
				{		
					...
					{        
						...
						{												
							...
							if($use) $use = ' - '.ucwords(strtolower($use));
							...
							$markup.='<option value="'.$_obj_floor->get_room_barcode().'"'.$selected.'>'.$_obj_floor->get_room_id().$use.'</option>'.PHP_EOL;
						}
					}
